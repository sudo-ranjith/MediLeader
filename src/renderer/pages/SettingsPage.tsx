...
export default function SettingsPage() {
  ...
  useEffect(() => {
    loadSettings();
    loadUsers();
    window.api.getVersion().then(v => setAppVersion(String(v ?? '')));
    window.api.getUserDataPath().then(p => setDataPath(String(p ?? '')));
  }, []);
  ...
  async function loadUsers() {
    if (!token) return;
    const res = await window.api.authListUsers(token);
  ...
  async function saveSettings() {
    ...
    setSaveMsg('');
    try {
      const res = await window.api.settingsSetBulk(settings);
      if (res.success) {
  ...
  async function handleAddUser(e: React.FormEvent) {
    ...
    const res = await window.api.authCreateUser(newUser, token);
    if (!res.success) { setAddUserError(res.error ?? 'Failed to add user'); return; }
  ...
  async function toggleUserActive(userId: number) {
    ...
    if (!token) return;
    const res = await window.api.authToggleUser(userId, token);
  ...
  async function handleManualSync() {
    ...
    setSyncMsg('');
    try {
      const res = await window.api.syncManual(token);
      if (res.success) {
        setSyncMsg(`Sync complete — pulled: ${res.data?.pulled ?? 0}, pushed: ${res.data?.pushed ?? 0}`);
        loadSettings();
      } else {
  ...
  async function handleBackupExport() {
    ...
    setBackupMsg(res.success ? 'Backup exported to: ' + res.path : 'Export failed: ' + res.error);
  }
  ...
  async function handleBackupImport() {
    ...
    if (res.cancelled) return;
    if (!res.success) setBackupMsg('Import failed: ' + res.error);
  }
    ...
    <div className="max-w-3xl space-y-4">
        ...
        <div className="card space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input className="input font-mono" value={settings.pharmacy_gstin || ''} onChange={e => updateSetting('pharmacy_gstin', e.target.value)} placeholder="33XXXXX..." />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Drug License No.</label>
              <input className="input" value={settings.pharmacy_dl_number || ''} onChange={e => updateSetting('pharmacy_dl_number', e.target.value)} />
            </div>
            <div>
            ...
            <div>
              ...
              <select className="select" value={language} onChange={e => { const l = e.target.value as Lang; setLang(l); setLanguage(l); }}>
      ...
      {activeTab === 'backup' && (
        <div className="card space-y-5">
          ...
          {backupMsg && (
            <div className={`p-3 rounded-lg text-sm border ${backupMsg.includes('failed') ? 'bg-red-50 border-red-200 text-red-700' : 'bg-green-50 border-green-200 text-green-700'}`}>
              {backupMsg}
            </div>
          )}
          <div className="grid grid-cols-2 gap-4">
            <div className="border border-slate-200 rounded-xl p-5 space-y-3">
              <div className="w-10 h-10 bg-blue-100 rounded-lg flex items-center justify-center"><Download size={20} className="text-blue-600" /></div>
              <div className="font-semibold text-slate-800">Export Backup</div>
              <p className="text-sm text-slate-500">Save a complete copy of all data to a file.</p>
              <button onClick={handleBackupExport} className="btn-primary w-full"><Download size={15} /> Export Database</button>
            </div>
            <div className="border border-slate-200 rounded-xl p-5 space-y-3">
              ...
              <p className="text-sm text-slate-500">Restore from a backup file. The app will restart automatically.</p>
          ...
          <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg text-sm text-amber-700">
            <strong>Warning:</strong> Importing a backup will permanently replace all current data. Export a backup first.
          </div>
        </div>
      )}
      ...
      {activeTab === 'about' && (
        <div className="card space-y-4">
          <div className="flex items-center gap-3 mb-2">
            <div className="w-12 h-12 bg-blue-600 rounded-xl flex items-center justify-center"><Info size={24} className="text-white" /></div>
            <div>
              <div className="font-bold text-lg text-slate-800">MediLeader</div>
          ...
          <div className="space-y-3 text-sm text-slate-600">
            <div className="p-3 bg-slate-50 rounded-lg">
              ...
              <p>Offline-first pharmacy management system built for Tamil Nadu pharmacies. Manages medicines, stock, GST billing, purchase orders, and financial ledger.</p>
            </div>
            <div className="p-3 bg-slate-50 rounded-lg">
            ...
            <div className="p-3 bg-slate-50 rounded-lg">
              ...
              <p>Electron + React + SQLite (better-sqlite3) + Tailwind CSS + Vite</p>
            </div>
            <div className="text-center text-slate-400 text-xs mt-4">Made with care for Tamil Nadu pharmacies</div>
          </div>
        </div>
      )}
